...
# c0, c1, c2 = symbols('c_{0} c_{1} c_{2}')
# varphi = symbols('\varphi', cls=Function)
# varphi = c2*p**2+c1*q**2+c0*p*q
# print(latex(expand(P_bracket(1, varphi, p**2))))
# print(latex(expand(P_bracket(1, varphi, q**2))))
# print(latex(expand(P_bracket(1, varphi, p*q))))
# exit()
...
for i in [11]:
    ...
    for k in np.arange(i):
        ...
        res = C_kn(eta, k, i - k, f(p), g(q)).args
        res = Add(*[Abs(term) for term in res])
        sum += res
    simplify(sum)
    print(sum)
